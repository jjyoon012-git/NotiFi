"""Focused validation refinement around the promoted 30-candidate pool."""
...
def main() -> None:
    ...
    data = slice_candidate_data(
        prepare(args, "val", device, pool_top_k=30), 30
    )
    ...
    metrics = {}
    for action_weight in (0.15, 0.20, 0.25):
        for risk_weight in (0.20, 0.25, 0.30):
            adjusted = (
                base + action_weight * action_score + risk_weight * risk_score
            )
            for temperature in (0.35, 0.50, 0.65):
                for top_k in (3, 5, 8):
                    ...
                    key = (
                        f"pool30_a{int(action_weight * 1000):04d}"
    ...
    result = {
        "status": "validation_selected_expanded_semantic_pool_refined",
        "protocol": args.exp,
