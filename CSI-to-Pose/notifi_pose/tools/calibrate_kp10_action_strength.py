"""Validation-only action-prior strength refresh after KP10 action fusion."""
...
def main() -> None:
    ...
    metrics = {}
    for strength in (0.25, 0.30, 0.35, 0.40, 0.45):
        predicted = current + strength * low
        predicted = predicted - predicted[:, :, :1]
        name = f"strength_{int(strength * 100):03d}"
    ...
    result = {
        "status": "validation_selected_kp10_action_strength",
        "protocol": args.exp,
