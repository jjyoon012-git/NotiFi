...
#: 데이터셋 패키지 루트. 환경변수 NOTIFI_DATASET_ROOT 로 덮어쓸 수 있다.
def _default_dataset_root() -> Path:
    candidates = (
        Path(r"D:\NotiFi-3D\Dataset_Splits\NotiFi_CSI_GVHMR_v2_LOSO_60_15_25"),
        Path(r"D:\mhw\Dataset_Splits\NotiFi_CSI_GVHMR_v2_LOSO_60_15_25"),
    )
    return next((path for path in candidates if path.exists()), candidates[0])


DATASET_ROOT = Path(os.environ.get("NOTIFI_DATASET_ROOT", _default_dataset_root()))
...
TIMESTAMP_ROOT = Path(
    os.environ.get(
        ...
        r"D:\NotiFi-3D\Timestamp_Upload_Staging\timestamp",
    )
)
TIMESTAMP_MANIFEST_CSV = Path(
    os.environ.get(
        "NOTIFI_TIMESTAMP_MANIFEST",
        r"D:\NotiFi-3D\Timestamp_Upload_Staging\audit\timestamp_manifest.csv",
    )
)
...
#: (표본 실측: mhw 0.76%, ajh 0.29%, lmh 0.01%, yja 0.00%).
...
#: GT 스키마. yja 789개만 smpl_full (SMPL 파라미터 추가), 나머지는 joints only.
#: **공통 계약은 joints_world / transl / frame_index 3개뿐이다.** SMPL 파라미터를
#: 학습 필수 경로에 넣으면 yja 가 빠지는 fold 에서 코드가 깨진다.
