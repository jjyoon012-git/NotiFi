...
from notifi_pose.cal17 import (
    ...
    procrustes_transport_class_prototypes,
...
from notifi_pose.pose_semantic_teacher import paired_pose_distillation_loss
from scripts.train_cal20_source_folds import (
    cal12_site_selection_score,
    counterfactual_view_consistency,
    experiment_name,
    group_dro_objective,
    load_clean_source_state,
    nested_site_split,
    teacher_student_view_consistency,
    update_ema_teacher,
    validate_training_options,
)
from notifi_pose.cal27 import (
    anchor_reconstruction_error,
    kernel_transport_query,
    kernel_transported_logits,
)
...
class CalibrationModelTests(unittest.TestCase):
    ...
    def test_deployment_runs_without_query_labels_or_gt(self) -> None:
        ...
        self.assertTrue(bool(domain_rejected["abstain"].item()))
        dual_bundle = dict(bundle)
        dual_bundle["action_ensemble_config"] = [{
            "source_outer_fold": "test",
            "linear_config": dict(bundle["action_config"]),
            "kernel_config": {
                "strength": 1.0, "kernel_temperature": 0.2,
                "regularization": 0.1, "prototype_temperature": 0.1,
                "site_temperature": 0.1, "mixture": 0.5,
            },
            "kernel_weight": 0.5,
        }]
        dual_bundle["risk_profiles"] = {
            "conservative": {"danger_bias": 0.0},
            "safety": {"danger_bias": 1.25},
        }
        dual = CAL20Deployment(dual_bundle, device="cpu")
        dual_calibration = dual.calibrate(
            self.support, self.support_mask, self.support_labels,
            self.absence, self.absence_mask,
        )
        conservative = dual.predict(
            self.query[:1], self.query_mask[:1], dual_calibration,
            simulate_pose=False, risk_profile="conservative",
        )
        safety = dual.predict(
            self.query[:1], self.query_mask[:1], dual_calibration,
            simulate_pose=False, risk_profile="safety",
        )
        self.assertEqual(conservative["risk_profile"], "conservative")
        self.assertEqual(safety["risk_profile"], "safety")
        self.assertGreater(
            float(safety["risk_logits"][0, 2]),
            float(conservative["risk_logits"][0, 2]),
        )
        with self.assertRaisesRegex(ValueError, "unknown risk profile"):
            dual.predict(
                self.query[:1], self.query_mask[:1], dual_calibration,
                simulate_pose=False, risk_profile="invalid",
            )
        deep_bundle = dict(dual_bundle)
        deep_bundle["deep_action"] = {
            "model": model.state_dict(),
            "model_config": model.model_config(),
            "source_library": list(bundle["source_library"]),
            "configs": [{
                "source_outer_fold": "test",
                "primary_linear_config": dict(bundle["action_config"]),
                "secondary_linear_config": dict(bundle["action_config"]),
            }],
            "ensemble_weights": [0.5, 0.5],
            "calibration_geometry_threshold": -1.0,
        }
        deep = CAL20Deployment(deep_bundle, device="cpu")
        deep_calibration = deep.calibrate(
            self.support, self.support_mask, self.support_labels,
            self.absence, self.absence_mask,
        )
        deep_output = deep.predict(
            self.query[:1], self.query_mask[:1], deep_calibration,
            simulate_pose=False, risk_profile="safety",
        )
        self.assertIsNotNone(deep_calibration.secondary_anchors)
        self.assertTrue(bool(deep_calibration.domain_pass))
        self.assertFalse(bool(deep_calibration.secondary_domain_pass))
        self.assertFalse(bool(
            deep_output["calibration_domain_pass"].item()
        ))
        self.assertTrue(bool(deep_output["abstain"].item()))
        self.assertEqual(
            tuple(deep_output["action_logits"].shape), (1, C.N_CLASSES)
        )
        self.assertTrue(torch.isfinite(deep_output["action_logits"]).all())
    ...
    def test_deployment_rejects_incomplete_support(self) -> None:
        ...
        with self.assertRaisesRegex(ValueError, "equal batches"):
            runtime.calibrate(
                self.support[:-1], self.support_mask[:-1],
                self.support_labels, self.absence, self.absence_mask,
            )

    def test_deployment_rejects_misaligned_or_nonfinite_csi(self) -> None:
        CAL20Deployment._validate_trial_batch(
            "query", self.query, self.query_mask
        )
        with self.assertRaisesRegex(ValueError, "axes do not match"):
            CAL20Deployment._validate_trial_batch(
                "query", self.query, self.query_mask[:, :-1]
            )
        invalid = self.query.clone()
        invalid[0, 0, 0, 0, 0] = float("nan")
        with self.assertRaisesRegex(ValueError, "non-finite"):
            CAL20Deployment._validate_trial_batch(
                "query", invalid, self.query_mask
            )
    ...
    def test_procrustes_transport_preserves_identity_and_target_anchors(self) -> None:
        classes = torch.randn(C.N_CLASSES, 8)
        source = torch.nn.functional.normalize(classes[:4], dim=-1)
        identity = procrustes_transport_class_prototypes(
            classes, source, source, (0, 1, 2, 3), 1.0, 1.0
        )
        torch.testing.assert_close(
            identity, torch.nn.functional.normalize(classes, dim=-1),
            atol=1e-5, rtol=1e-5,
        )
        target = torch.randn_like(source)
        transported = procrustes_transport_class_prototypes(
            classes, source, target, (0, 1, 2, 3), 1.0, 0.25
        )
        torch.testing.assert_close(
            transported[:4], torch.nn.functional.normalize(target, dim=-1)
        )

    def test_kernel_transport_identity_and_anchor_reconstruction(self) -> None:
        target = torch.nn.functional.normalize(torch.randn(9, 16), dim=-1)
        identity = kernel_transport_query(
            target, target, target, 1.0, 0.2, 0.01,
        )
        torch.testing.assert_close(identity, target, atol=1e-5, rtol=1e-5)
        source = torch.nn.functional.normalize(
            target + 0.05 * torch.randn_like(target), dim=-1,
        )
        error = anchor_reconstruction_error(
            target, source, 1.0, 0.1, 1e-4,
        )
        self.assertLess(float(error), 1e-5)

    def test_kernel_transport_logits_have_action_shape(self) -> None:
        target = {
            "embedding": torch.randn(4, 16),
            "anchors": torch.randn(9, 16),
        }
        source = [{
            "classes": torch.randn(C.N_CLASSES, 16),
            "anchors": torch.randn(9, 16),
        }]
        logits = kernel_transported_logits(
            target, source, 0.5, 0.2, 0.1, 0.1, 0.1,
        )
        self.assertEqual(logits.shape, (4, C.N_CLASSES))
    ...
    def test_optional_calibration_modules_preserve_shared_initialization(self) -> None:
        torch.manual_seed(90210)
        baseline = CAL20RelativeMotionDG(hidden=16, width=32, domains=3)
        baseline_state = baseline.state_dict()
        torch.manual_seed(90210)
        augmented = CAL20RelativeMotionDG(
            hidden=16,
            width=32,
            domains=3,
            learned_support_matcher=True,
            hierarchical_action_heads=True,
            support_film=True,
        )
        augmented_state = augmented.state_dict()

        optional_prefixes = (
            "motion_matcher.", "state_matcher.", "coarse_head.",
            "start_head.", "support_film_head.",
        )
    ...
    def test_adaptive_subcarrier_groups_start_as_exact_no_op(self) -> None:
        torch.manual_seed(771)
        baseline = SubcarrierBranch(6, hidden=16, dropout=0.0).eval()
        torch.manual_seed(771)
        grouped = SubcarrierBranch(
            6, hidden=16, dropout=0.0, adaptive_groups=4
        ).eval()
        values = torch.randn(2, 9, 3, 20, 6)
        with torch.no_grad():
    ...
    def test_episode_link_dropout_uses_one_shared_tx(self) -> None:
        support = torch.ones(2, 8, 3, 4, 2)
        query = torch.full((3, 8, 3, 4, 2), 2.0)
        support_mask = torch.ones(2, 8, 3, dtype=torch.bool)
        query_mask = torch.ones(3, 8, 3, dtype=torch.bool)
        dropped = drop_episode_link([
            (support, support_mask), (query, query_mask)
        ], seed=71)
        missing_support = (~dropped[0][1].any((0, 1))).nonzero().flatten()
        missing_query = (~dropped[1][1].any((0, 1))).nonzero().flatten()
        self.assertEqual(len(missing_support), 1)
        torch.testing.assert_close(missing_support, missing_query)
        link = int(missing_support.item())
        self.assertTrue(bool((dropped[0][0][:, :, link] == 0.0).all()))
        self.assertTrue(bool((dropped[1][0][:, :, link] == 0.0).all()))
    ...
    def test_subcarrier_mask_is_shared_and_replaced_by_static_baseline(self) -> None:
        absence = torch.empty(2, 5, 3, 20, 2)
        absence[..., 0] = 2.0
        absence[..., 1] = 0.3
        query = torch.randn(3, 5, 3, 20, 2)
        query[..., 0].abs_().add_(0.5)
        absence_mask = torch.ones(2, 5, 3, dtype=torch.bool)
        query_mask = torch.ones(3, 5, 3, dtype=torch.bool)

        (_, _), (masked_query, masked_query_mask) = mask_subcarrier_band(
            [(absence, absence_mask), (query, query_mask)],
            seed=13,
            fraction=0.25,
        )

        changed = (masked_query != query).any(dim=(0, 1, 2, 4))
        self.assertEqual(int(changed.sum()), 5)
        torch.testing.assert_close(
            masked_query[..., changed, 0],
            torch.full_like(masked_query[..., changed, 0], 2.0),
        )
        torch.testing.assert_close(
            masked_query[..., changed, 1],
            torch.full_like(masked_query[..., changed, 1], 0.3),
        )
        self.assertTrue(torch.equal(masked_query_mask, query_mask))
    ...
    def test_invalid_physical_invariance_options_fail_before_training(self) -> None:
        ...
        options.motion_phase_bins = 8
        options.reflection_probability = 0.25
        options.class_aligned_cross_subject = True
        options.cross_subject_pairing = False
        with self.assertRaisesRegex(ValueError, "requires cross_subject_pairing"):
            validate_training_options(options)

    def test_group_dro_upweights_the_harder_source_site(self) -> None:
        losses = torch.tensor((0.5, 2.0), requires_grad=True)
        groups = torch.tensor((0, 1))
        weights = torch.full((2,), 0.5)
        objective = group_dro_objective(
            losses, groups, weights, eta=0.1
        )
        self.assertGreater(float(weights[1]), float(weights[0]))
        objective.backward()
        self.assertGreater(float(losses.grad[1]), float(losses.grad[0]))

    def test_counterfactual_consistency_is_zero_for_identical_views(self) -> None:
        payload = {
            "action_logits": torch.randn(4, C.N_CLASSES),
            "risk_logits": torch.randn(4, C.N_RISK),
            "embedding": torch.randn(4, 16),
        }
        loss = counterfactual_view_consistency(payload, payload)
        self.assertLess(abs(float(loss)), 1e-6)

    def test_ema_teacher_consistency_and_update(self) -> None:
        payload = {
            "action_logits": torch.randn(4, C.N_CLASSES),
            "risk_logits": torch.randn(4, C.N_RISK),
            "embedding": torch.randn(4, 16),
        ...
        self.assertLess(
            abs(float(teacher_student_view_consistency(payload, payload))),
            1e-6,
        )
        ...
        update_ema_teacher(teacher, student, decay=0.75)
        torch.testing.assert_close(
            teacher.weight, torch.full_like(teacher.weight, 0.5)
        )

    def test_paired_pose_distillation_is_zero_for_identical_outputs(self) -> None:
        embedding = torch.randn(4, 16)
        action = torch.randn(4, C.N_CLASSES)
        risk = torch.randn(4, C.N_RISK)
        teacher = {
            "embedding": embedding,
            "action_logits": action,
            "risk_logits": risk,
        }
        loss = paired_pose_distillation_loss(
            embedding, action, risk, teacher
        )
        self.assertLess(abs(float(loss)), 1e-6)
    ...
    def test_support_film_starts_as_exact_no_op(self) -> None:
        model = CAL20RelativeMotionDG(
            hidden=16,
            width=32,
            domains=4,
        ...
        torch.testing.assert_close(
            output["action_logits"], output["base_action_logits"],
            rtol=0.0, atol=0.0,
        )
        torch.testing.assert_close(
            output["risk_logits"], output["base_risk_logits"],
            rtol=0.0, atol=0.0,
        )

    def test_latent_support_normalization_starts_as_exact_no_op(self) -> None:
        torch.manual_seed(810)
        baseline = CAL20RelativeMotionDG(
            hidden=16, width=32, domains=4, dropout=0.0,
            use_doppler=False,
        ).eval()
        torch.manual_seed(810)
        normalized = CAL20RelativeMotionDG(
            hidden=16, width=32, domains=4, dropout=0.0,
            use_doppler=False, latent_support_normalization=True,
        ).eval()
        with torch.no_grad():
            expected = baseline(
                self.query, self.query_mask,
        ...
        torch.testing.assert_close(
            actual["action_logits"], expected["action_logits"],
            rtol=0.0, atol=0.0,
        )
        self.assertTrue(bool((actual["latent_support_gate"] == 0.0).all()))

    def test_latent_support_normalization_reaches_classification_path(self) -> None:
        model = CAL20RelativeMotionDG(
            hidden=16, width=32, domains=4, dropout=0.0,
            use_doppler=False, latent_support_normalization=True,
        ).eval()
        with torch.no_grad():
            ...
            model.latent_support_gate.fill_(0.5)
            normalized = model(
                self.query, self.query_mask,
                self.support, self.support_mask, self.support_labels,
                self.absence, self.absence_mask,
            )["action_logits"]
        self.assertGreater(float((normalized - baseline).abs().max()), 1e-5)

    def test_compositional_action_fusion_starts_as_no_op_then_changes_logits(self) -> None:
        model = CAL20RelativeMotionDG(
            hidden=16, width=32, domains=4, dropout=0.0,
            use_doppler=False,
            hierarchical_action_heads=True,
            compositional_action_fusion=True,
        ).eval()
        with torch.no_grad():
            initial = model(
                self.query, self.query_mask,
            ...
            torch.testing.assert_close(
                initial["action_logits"], initial["base_action_logits"],
                rtol=0.0, atol=0.0,
            )
            model.composition_gate.fill_(0.5)
            fused = model(
                self.query, self.query_mask,
                self.support, self.support_mask, self.support_labels,
                self.absence, self.absence_mask,
        ...
        self.assertGreater(
            float((fused["action_logits"] - initial["action_logits"]).abs().max()),
            1e-5,
        )

    def test_motion_salience_attention_starts_as_exact_no_op(self) -> None:
        ...
        torch.manual_seed(811)
        salience = CAL20RelativeMotionDG(
            hidden=16, width=32, domains=4, dropout=0.0,
            use_doppler=False, motion_salience_attention=True,
        ).eval()
        with torch.no_grad():
            expected = baseline(
                self.query, self.query_mask,
        ...
        torch.testing.assert_close(
            actual["motion_salience"].sum(1), torch.ones(len(self.query)),
        )
    ...
    def test_model_factory_restores_cal20(self) -> None:
        source = CAL20RelativeMotionDG(
            ...
            hierarchical_action_heads=True,
            support_film=True,
            adaptive_subcarrier_groups=4,
            latent_support_normalization=True,
        )
        ...
        self.assertTrue(restored.learned_support_matcher)
        self.assertTrue(restored.hierarchical_action_heads)
        self.assertTrue(restored.support_film)
        self.assertEqual(restored.adaptive_subcarrier_groups, 4)
        self.assertTrue(restored.latent_support_normalization)
