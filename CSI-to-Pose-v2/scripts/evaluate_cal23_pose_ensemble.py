...
def oracle_action_logits(labels: torch.Tensor) -> torch.Tensor:
    """진단 전용으로 정답 행동 하나에만 확률 질량을 둔 logit을 만든다."""
    logits = torch.full(
        (len(labels), C.N_CLASSES), -20.0,
...
def main() -> None:
    ...
    for held_out in ("ajh", "mhw", "lmh"):
        ...
        oracle_payloads = [
            retrieval_payload(
                embedded[site], oracle_action_logits(embedded[site]["labels"]),
                candidate_pose, candidate_valid, candidate_descriptor,
                normalized_candidates, candidate_labels, center, scale,
                pose_array, valid_array,
            )
            for site in outer_sites
        ]
        oracle_prediction, oracle_target, oracle_valid, oracle_risk = (
            ensemble_prediction(
                oracle_payloads,
                selected_config["neighbors"],
                selected_config["temperature"],
        ...
        folds[held_out] = {
            ...
            "oracle_action_pose_diagnostic": retrieval_metrics(
                oracle_prediction, oracle_target, oracle_valid, oracle_risk
            ),
            ...
            "oracle_action_used_for_model_selection": False,
        }
    result = {
        ...
        "diagnostic_query_action_labels_used": True,
        "diagnostic_is_deployment_performance": False,
        "absence_trials": options.absence_trials,
    }
