...
def source_geometry_threshold(
    source_library: list[dict[str, torch.Tensor]],
) -> tuple[float, list[float]]:
    """각 source site의 최근접 anchor geometry 거리로 OOD threshold를 만든다."""
    if len(source_library) < 2:
        raise ValueError("geometry threshold requires at least two source sites")
    nearest = []
    ...
    return 1.5 * max(nearest), nearest
...
def main() -> None:
    ...
    parser.add_argument("--fixed-dual-result", type=Path)
    parser.add_argument("--deep-action-run", type=Path)
    parser.add_argument("--fixed-deep-action-result", type=Path)
    ...
    fixed_dual = None
    if options.fixed_dual_result is not None:
        fixed_dual = json.loads(
            options.fixed_dual_result.read_text(encoding="utf-8")
        )
        require_source_clean(fixed_dual, "fixed dual result")
        if fixed_dual.get("outer_used_for_selection") is not False:
            raise RuntimeError("fixed dual result used outer labels for selection")
    fixed_deep = None
    if options.fixed_deep_action_result is not None:
        if options.deep_action_run is None:
            raise ValueError("fixed deep action result requires --deep-action-run")
        fixed_deep = json.loads(
            options.fixed_deep_action_result.read_text(encoding="utf-8")
        )
        require_source_clean(fixed_deep, "fixed deep action result")
        if fixed_deep.get("outer_holdout_used_for_selection") is not False:
            raise RuntimeError("fixed deep action result used outer labels")
    elif options.deep_action_run is not None:
        raise ValueError("--deep-action-run requires --fixed-deep-action-result")
    ...
    if options.deep_action_run is not None:
        secondary_checkpoint = torch.load(
            options.deep_action_run / "deployment_model.pt",
            map_location="cpu", weights_only=False,
        )
        for key in (
            ...
            if secondary_checkpoint.get(key) is not False:
                raise RuntimeError(f"deep action checkpoint is not clean: {key}")
        secondary_model = build_calibration_model(
            secondary_checkpoint["model_config"]
        ).to(device)
        secondary_model.load_state_dict(secondary_checkpoint["model"])
    ...
    secondary_source_library = None
    if secondary_model is not None:
        secondary_embedded = {
            site: embed_site(
                secondary_model, store, index, selected_rows, sites,
                site, device, absence_trials=options.absence_trials,
            )
            for site in all_sites
        }
        secondary_source_library = [{
            "site": site,
            "classes": class_prototypes(secondary_embedded[site]).cpu(),
            "anchors": secondary_embedded[site]["anchors"].cpu(),
        } for site in all_sites]
    geometry_threshold, nearest_geometry = source_geometry_threshold(
        source_library
    )
    secondary_geometry_threshold = None
    secondary_nearest_geometry = None
    if secondary_source_library is not None:
        secondary_geometry_threshold, secondary_nearest_geometry = (
            source_geometry_threshold(secondary_source_library)
        )
    ...
    bundle = {
        "bundle_version": (
            ...
            if fixed_deep is not None
            else "cal32_fixed_action_safety_risk_v1"
            if fixed_dual is not None else "cal20_cal17_cal23_v4"
        ),
        ...
        "provenance": {
            ...
            "fixed_dual_result_sha256": (
                sha256(options.fixed_dual_result)
                if options.fixed_dual_result is not None else None
            ),
            "deep_action_model_sha256": (
                sha256(options.deep_action_run / "deployment_model.pt")
                if options.deep_action_run is not None else None
            ),
            "fixed_deep_action_result_sha256": (
                sha256(options.fixed_deep_action_result)
                if options.fixed_deep_action_result is not None else None
            ),
        },
    }
    if fixed_dual is not None:
        ...
        for held_out in ("ajh", "mhw", "lmh"):
            config = fixed_dual["fixed_configs"][held_out]
            linear = dict(zip((
            ...
            action_ensemble.append({
                "source_outer_fold": held_out,
                "linear_config": linear,
                "kernel_config": kernel,
                "kernel_weight": float(config["kernel_weight"]),
            })
            danger_biases.append(float(
                config.get("native_risk_danger_bias", 0.0)
            ))
        bundle["action_ensemble_config"] = action_ensemble
        bundle["risk_profiles"] = {
            "conservative": {"danger_bias": 0.0},
            "safety": {"danger_bias": float(np.median(danger_biases))},
        }
    if fixed_deep is not None:
        names = (
            "strength", "anchor_temperature", "prototype_temperature",
        ...
        deep_configs = []
        for held_out in ("ajh", "mhw", "lmh"):
            primary, secondary = fixed_deep[
                "fixed_linear_configs"
            ][held_out]
            deep_configs.append({
                "source_outer_fold": held_out,
                "primary_linear_config": dict(zip(names, primary)),
                "secondary_linear_config": dict(zip(names, secondary)),
            })
        bundle["deep_action"] = {
            "model": secondary_checkpoint["model"],
            "model_config": secondary_checkpoint["model_config"],
            "source_library": secondary_source_library,
            "configs": deep_configs,
            "ensemble_weights": [0.5, 0.5],
            "calibration_geometry_threshold": secondary_geometry_threshold,
            "source_leave_one_site_geometry": secondary_nearest_geometry,
        }
        bundle["risk_profiles"] = {
            "conservative": {"danger_bias": 0.0},
            "safety": {"danger_bias": float(fixed_deep["danger_bias"])},
        }
    ...
    print(json.dumps({
        ...
        "pose_config": pose_config,
        "calibration_geometry_threshold": geometry_threshold,
        "secondary_calibration_geometry_threshold": secondary_geometry_threshold,
        "target_subject_used": False,
        "sealed_yja_used": False,
        "absence_trials": options.absence_trials,
