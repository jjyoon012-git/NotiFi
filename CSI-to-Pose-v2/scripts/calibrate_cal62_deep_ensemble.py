...
def main() -> None:
    ...
    parser.add_argument(
        "--dual-transport", action="store_true",
        help="각 encoder에 CAL17+CAL27을 적용한 뒤 확률 ensemble",
    )
    ...
    for held_out in ("ajh", "mhw", "lmh"):
        ...
        linear_configs = [
            choose_linear_config(
                [payload[site] for site in inner_sites], library
            )
            for payload, library in zip(embedded, libraries)
        ]
        kernel_configs = [None, None]
        kernel_weights = [0.0, 0.0]
        if options.dual_transport:
            kernel_configs = [
            ...
            kernel_weights = [
                ...
                for payload, library, linear, kernel in zip(
                    embedded, libraries, linear_configs, kernel_configs
                )
            ]

        def components_for(site_names: list[str]) -> list[dict[str, torch.Tensor]]:
            actions = []
            for payload, library, linear, kernel, kernel_weight in zip(
                embedded, libraries, linear_configs,
                kernel_configs, kernel_weights,
            ):
                targets = [payload[site] for site in site_names]
                if kernel is None:
                    _, current = evaluate_linear(targets, library, linear)
                else:
                    current = blended_actions(
                        targets, library, linear, kernel, kernel_weight
                    )
        ...
        folds[held_out] = {
            ...
            "linear_configs": [list(config) for config in linear_configs],
            "kernel_configs": [
    ...
    result = {
        ...
        "dual_transport": options.dual_transport,
