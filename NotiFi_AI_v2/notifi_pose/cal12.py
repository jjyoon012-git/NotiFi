...
class SubcarrierBranch(nn.Module):
    ...
    def __init__(
        ...
        hidden: int,
        dropout: float,
        adaptive_groups: int = 0,
    ):
        super().__init__()
        spectral = max(24, hidden // 2)
        groups = 8 if spectral % 8 == 0 else 1
        self.hidden = int(hidden)
        self.adaptive_groups = int(adaptive_groups)
        ...
        if self.adaptive_groups >= 2:
            optional_rng = torch.random.get_rng_state()
            try:
                self.group_score = nn.Sequential(
                    nn.LayerNorm(spectral),
                    nn.Linear(spectral, max(spectral // 2, 8)),
                    nn.GELU(),
                    nn.Linear(max(spectral // 2, 8), 1),
                )
                self.group_projection = nn.Linear(spectral, hidden)
                nn.init.zeros_(self.group_projection.weight)
                nn.init.zeros_(self.group_projection.bias)
                self.group_gate = nn.Parameter(torch.tensor(-2.0))
            finally:
            ...
            self.register_buffer(
                "group_gate", torch.tensor(-2.0), persistent=False
            )
    ...
    def forward(self, values: torch.Tensor) -> torch.Tensor:
        ...
        for chunk in torch.split(flat, 4096):
            ...
            output = self.projection(torch.cat((
                encoded.mean(-1), encoded.amax(-1)
            ), dim=-1))
            if self.group_score is not None and self.group_projection is not None:
                group_tokens = F.adaptive_avg_pool1d(
                    encoded, self.adaptive_groups
                ).transpose(1, 2)
                group_weight = torch.softmax(
                    self.group_score(group_tokens).squeeze(-1), dim=-1
                )
                grouped = (
                    group_tokens * group_weight[..., None]
                ).sum(1)
                output = output + torch.sigmoid(
                    self.group_gate
                ) * self.group_projection(grouped)
            pooled.append(output)
        return torch.cat(pooled).reshape(
